...
RLSolver is an open-source RL-based solver for NP-hard problems. RLSolver uses reinforcement learning (RL) or deep learning (DL) to automate the search process of combinatorial optimization. It uses auto-regressive neural networks, or auto-regressive graph neural networks (GNNs) as the policy network. RLSolver provides open datasets and benchmarks, including classical methods, state-of-the-art algorithms, our new algorithms, and well-known solvers (Gurobi, SCIP, etc.).

We aim to showcase that reinforcement learning (RL) or machine learning (ML) with GPUs delivers the best benchmark performance for large-scale nonconvex and NP-complete problems. When the size of these problems becomes large, it is very hard to obtain optimal or near optimal solutions. RL with the help of GPU computing can obtain high-quality solutions within short time. Take the graph maxcut problem as an example, when the number of nodes is up to 10,000, conventional solvers may not obtain high-quality solutions. RLSolver obtains almost the same or better solutions than the commercial solver Gurobi, and with a speedup of 30x ~ 60x.

RLSolver collects RL/ML tricks for solving nonconvex and NP-complete optimization problems. It provides open datasets and benchmarks, including calssical methods, popular algorithms, our new algorithms, and well-known solvers. We have provided functions to read data and write results; therefore, it is convenient to compare the performance if users want to play this project. We encourage users to try RL/ML tricks, and implement their own new ideas based on the datasets and benchmarks. If encountering any problems, please submit github issues, and we can communicate with eath other there. 
