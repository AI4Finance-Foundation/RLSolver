...
class MaxcutEnv():
    ...
    def generate_adjacency_symmetric_matrix_batch(self, sparsity, if_binary, CL=False, graph_type='ER'): # sparsity for binary
        if if_binary:
            self.upper_triangle = (torch.rand(self.num_env, self.N, self.N) < sparsity).int().triu(diagonal=1)
        else:
            self.upper_triangle = torch.mul(torch.rand(self.num_env, self.N, self.N).triu(diagonal=1), (torch.rand(self.num_env, self.N, self.N) < self.sparsity).int().triu(diagonal=1))
    ...
    def get_cut_value(self,configuration):
        return torch.mul(torch.bmm(configuration.reshape(-1, self.N, 1), (1 - configuration.reshape(-1, self.N, 1)).transpose(-1, -2)), self.adjacency_matrix).flatten(start_dim=1).sum(dim=-1)
