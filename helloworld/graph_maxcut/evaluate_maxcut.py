...
def evaluator(policy_net_maxcut, N=4, device=torch.device("cuda:0")):
    ...
    with open ("s2v_dqn_benchmark/gtype-erdos_renyi-nrange-15-n_graph-164-p-0.15-m-0-w-float-0-1-cnctd-0-seed-2.pkl", 'rb') as f:
        test_adjacency_matrix = torch.as_tensor(np.array(pkl.load(f, encoding = 'latin1')), dtype=torch.float).to(device)
    state = env_maxcut.reset(if_test=True, test_adjacency_matrix=test_adjacency_matrix)
    cut_value = torch.zeros(state[0].shape[0], env_maxcut.episode_length)
    while(1):
        action = policy_net_maxcut(state)
        action = torch.where(action >= 0.5, 1.0, 0.)
