...
class Config:
    def __init__(self, agent_class=None, env_class=None, env_args=None):
        ...
        self.batch_size = int(4096)  # num of transitions sampled from replay buffer.
...
class AgentDQN(AgentBase):
    ...
    def explore_env(self, env, horizon_len: int, if_random: bool = False) -> [Tensor]:
        ...
        ary_state = self.states.flatten()
        get_action = self.act.get_action
        for i in range(horizon_len):
            state = torch.as_tensor(ary_state, dtype=torch.float32, device=self.device)
            #print(horizon_len, i, state.shape)
    ...
    def update_net(self, buffer) -> [float]:
        ...
        update_times = 1 #int(buffer.cur_size * self.repeat_times / self.batch_size)
...
def get_rewards_and_steps(env, actor, if_render: bool = False) -> (float, int):  # cumulative_rewards and episode_steps
    ...
    for episode_steps in range(12345):
        ...
        if done:
            break
    # print(episode_steps, cumulative_returns)
...
env_config = {'N': 5,
              ...
              'action_dim': 5, 
              'if_discrete': True}
args = Config(agent_class=AgentDQN, env_class=or_gym.make, env_args=env_config)  # see `Config` for explanation
args.break_step = int(2e5)  # break training if 'total_step > break_step'
args.net_dims = (64, 32)  # the middle layer dimension of MultiLayer Perceptron
args.gamma = 0.95  # discount factor of future rewards
