...
def check_simulator():
    ...
    simulator = SimulatorGraphMaxCut(sim_name=graph_name, graph=graph, device=device)
...
def find_best_num_sims():
    ...
    graph = load_graph(graph_name=graph_name)
    device = th.device(f'cuda:{gpu_id}' if th.cuda.is_available() and gpu_id >= 0 else 'cpu')
    simulator = SimulatorGraphMaxCut(sim_name=graph_name, graph=graph, device=device, if_bidirectional=False)
