...
def train_curriculum_learning(policy_net_tsp, optimizer, save_path, device, N=4, num_epochs=400000,
    ...
    for epoch in pbar:
        ...
        if epoch % num_epochs_to_save_model == 0:
            th.save(policy_net_tsp.state_dict(), save_path + f"{epoch}.pth")
        ...
        if epoch % num_epochs_to_evaluate == 0:
            with th.no_grad():
                sum_rate = th.zeros(100, env_tsp.episode_length, 1)
                for i_p in range(1):
...
if __name__  == "__main__":
    ...
    optimizer = th.optim.Adam(policy_net_tsp.parameters(), lr=learning_rate)

    try:
        train_curriculum_learning(policy_net_tsp, optimizer, N=N, save_path=save_path, device=device,)
        th.save(policy_net_tsp.state_dict(), save_path + "policy_net_tsp_1.pth")  # number your result policy net
