...
class TSPEnv():
    def __init__(self, N=4, episode_length=6, num_env=4096, device=th.device("cuda:0")):
        self.N = N  # #antennas
        self.basis_vectors, _ = th.linalg.qr(th.rand(self.N * 2, self.N * 2, dtype=th.float))
        self.subspace_dim = 1
    ...
    def generate_graph_cl(self, shape):
        coordinates = th.rand(self.subspace_dim, 1, device=self.device)
        vec_H = th.matmul(self.basis_vectors[:self.subspace_dim].T, coordinates)
        tmp_Hx = th.mat_mul(vec_H[:self.N], th.ones(1, self.N, device=self.device))
        tmp_Hy = th.mat_mul(vec_H[self.N:], th.ones(1, self.N, device=self.device))
        mat_H = th.sqrt((tmp_Hx - tmp_Hx.T) ** 2 + (tmp_Hy - tmp_Hy.T) ** 2) + self.diagself.diag
        return mat_H

    def generate_graph_rand(self, shape):
        vec_H = th.rand(self.N * 2) * 2 - 1
        tmp_Hx = th.mat_mul(vec_H[:self.N], th.ones(1, self.N, device=self.device))
        tmp_Hy = th.mat_mul(vec_H[self.N:], th.ones(1, self.N, device=self.device))
        mat_H = th.sqrt((tmp_Hx - tmp_Hx.T) ** 2 + (tmp_Hy - tmp_Hy.T) ** 2) + self.diagself.diag
        return mat_H
