...
class Env():
    def __init__(self, N=4, episode_length=6, num_env=4096, max_dim=2, epsilon=0.9, device=torch.device("cuda:0")):
        self.N = N
        self.device = device
    ...
    def step(self, action):
        ...
        for k in range(action.shape[0]):
            ...
            for j in range(self.start[k, selected_edge_id + 1], self.end[k, selected_edge_id + 1] + 1):
                r *= (state[j, j] * state[j, self.start[k, selected_edge_id + 1] - 1] * state[self.end[k, selected_edge_id + 1] + 1, j])
...
def train_curriculum_learning(policy_net, optimizer, device, N=4, num_epochs=100000000, num_env=128, gamma=0.9, if_wandb=False):
    ...
    for epoch in range(num_epochs):
        ...
        loss = 0
        env.epsilon = max(0.5, 0.5 + 0.5 * (1 - epoch / 2000))
        while (1):
            ...
            if done and test == True:
                print(env.reward.sum().item() / env.num_env, env.reward_no_prob.sum().item() / env.num_env, epoch)
                if if_wandb:
