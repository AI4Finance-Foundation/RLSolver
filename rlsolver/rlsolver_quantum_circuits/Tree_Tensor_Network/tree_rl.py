...
class Env():
    def __init__(self, N=63, episode_length=6, num_env=100, max_dim=2, epsilon=0.9, device=torch.device("cuda:0")):
        self.N = N
        def power_of_two(n):
            count = 0
            while n > 1:
...
class Policy_Net(nn.Module):
    def __init__(self, mid_dim=1024, N=63):
...
def train_curriculum_learning(policy_net, optimizer, device, N=63, num_epochs=100000000, num_env=100, gamma=0.9,
    ...
    for epoch in range(num_epochs):
        ...
        env.epsilon = max(0.9, 0.5 + 0.5 * (1 - epoch / 300))
        while (1):
