...
ThetaG14L10 = "2U"  # best_score 21  count 2
ThetaG14L12 = "9v"  # best_score 27  count 8
ThetaG14L14 = "7l"  # best_score 33  count 16
ThetaG14L16 = "Ey"  # best_score 38  count 192
ThetaG14L18 = "$F$"  # best_score 40  count 4
ThetaG14L20 = "3yF$"  # best_score 45  count 16
ThetaG14L22 = "BmF$"  # best_score 51  count 1024
ThetaG14L24 = "x0F$"  # best_score 59  count 1024
ThetaG14L26 = "3t$l$"  # best_score 68  count 1024
ThetaG14L28 = "Fh_F$"  # best_score 75  count 1024
ThetaG14L30 = "_RuF$"  # best_score 83  count 2048
...
class GraphMaxCutEnv:
    def __init__(self, graph_key: str = 'g14', gpu_id: int = -1, num_limit: int = th.inf):
    ...
    def get_objectives(self, probs):
        ...
        return -(p0s + p1s - 2 * p0s * p1s).sum(1)  # 计算节点不一定属于某个集合的分割距离，算出浮点数

    def get_scores(self, probs):
        p0s, p1s = self.get_p0s_p1s(probs)
        return (p0s ^ p1s).sum(1)  # 计算节点一定属于某个集合的分割距离，算出正整数
    ...
    def node_prob_bool_to_str(self, x_bool: TEN) -> str:
        ...
        return x_str.zfill(ceil(self.num_nodes // 6))
...
def exhaustion_search():
    ...
    for num_limit in range(10, 34, 2)[11:]:
        ...
        np.savez_compressed(f'exhaustion_{graph_key}_{num_limit:06}.npz', arr=all_score)
...
def read_all_scores():
    ...
    print()
    for num_limit in range(10, 34, 2)[11:]:
        scores = np.load(f'exhaustion_{graph_key}_{num_limit:06}.npz')['arr']
        ...
        print(f"ThetaG{graph_key[1:]}L{num_limit} = \"{best_theta}\"  # best_score {best_score}  count {num_best}")
...
def generate_theta_by_auto_regression(num_envs, device, dim, opt_opti):
    node_prob = th.rand((1, num_envs, 1), dtype=th.float32, device=device)
    sample = node_prob.squeeze(0)
    hidden = None
    ...
    entropies = th.stack(entropies)
    return thetas, logprobs, entropies


def train_optimizer_level4_generate_theta_by_auto_regression():
    gpu_id = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    num_envs = 2 ** 8
    graph_key = 'g14'
    # graph_key = 'g70'
    ...
    for i in range(num_opt):
        thetas, logprobs, entropies = generate_theta_by_auto_regression(num_envs, device, dim, opt_opti)
        # todo How to use entropy???
        scores = env.get_scores(probs=thetas).detach().to(th.float32)

        obj_probs = logprobs.exp()
        obj = ((obj_probs / obj_probs.mean()) * -scores).mean()
        ...
        if i % eval_gap == 0:
            _thetas, _logprobs, entropies = generate_theta_by_auto_regression(num_envs, device, dim, opt_opti)
