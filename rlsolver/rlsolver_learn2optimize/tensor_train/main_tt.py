...
def do_fit(dim, opt_net, meta_opt, objective, optimizee_class, unroll, optim_it, env, should_train=True):
    if should_train:
        opt_net.train()
    else:
        opt_net.eval()
    ...
    for iteration in range(1, optim_it + 1):
        ...
        for name, p in optimizee.all_named_parameters():
            ...
            temp = updates.view(*p.size())
            result_params[name] = temp/th.sum(temp)
...
def forward_pass(dim, opt_net, target, optimizee_class, optim_it, env):
    opt_net.eval()
...
def do_test(dim, best_loss, opt_net, objective, optimizee_class, optim_it, Ptest, test_data, env):
    loss = []
    lossMMSE = []
    ...
    for P in Ptest:
        for H in test_data:
            ...
            target = env
            #objective(Hscaled)
            rnn = forward_pass(dim, opt_net, target, optimizee_class, optim_it, env=env)
            loss.append(rnn)
...
def train_optimizer(env, dim, run_id, objective, optimizee_class, test_every=20, preproc=False, unroll=1, optim_it=3, lr=0.001, hidden_sz=20, load_net_path=None, Ntest=10, save_path=None, P_eval=None, Ptest=None, N_train_epochs=1000):
    N = dim
    K = 0
    test_data = load_test_data(N, K, Ntest)

    opt_net = w(Policy_Net(preproc=preproc, hidden_sz=hidden_sz))
    # summary(opt_net)
    ...
    # best_loss, _, _ = do_test(dim, [0]*len(Ptest), opt_net, objective, optimizee_class=optimizee_class, optim_it=200,  test_data=test_data,  Ptest=Ptest, env=env)
    best_loss = 0
    history = {
    ...
    for epoch in range(1, N_train_epochs+1):
        ...
        loss = do_fit(dim, opt_net, meta_opt, env, optimizee_class, unroll, optim_it, should_train=True, env=env)

        history['train_loss'].append(-np.mean(loss))
        if epoch % test_every == 0:
            print('='*60)
            print('epoch',epoch)
            loss, _, _ = do_test(dim, best_loss, opt_net, objective, optimizee_class=optimizee_class, optim_it=3, test_data=test_data, Ptest=Ptest, env=env)
...
class OptimizeeTT(MetaModule):
    def __init__(self, N):
        super().__init__()
        self.N = N - 1
        self.register_buffer('theta', w(to_var(th.ones(self.N,device=device), requires_grad=True)))
        # self.theta = self.theta.to(device)
        # for name, p in self.all_named_parameters():
        #     p = p.to(device)
    def forward(self, target):
...
class Policy_Net(nn.Module):
    def __init__(self, N=4,preproc=False, hidden_sz=20, preproc_factor=10.0 ):
        super(Policy_Net, self).__init__()
        self.N = N + 2
        self.action_dim = N - 1
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.hidden_sz = hidden_sz
        self.input_dim = (N + 2) * (N + 2) + N + N + (N - 1) + (N - 1)
        if preproc:
            self.recurs = nn.LSTMCell(2, hidden_sz)
        else:
            self.recurs = nn.LSTMCell(self.input_dim, hidden_sz)
        self.recurs2 = nn.LSTMCell(hidden_sz, hidden_sz)
        self.output = nn.Linear(hidden_sz, 1)
        self.preproc = preproc
        self.preproc_factor = preproc_factor
    ...
    def forward(self, inp, hidden, cell):
        # inp = th.cat((s.reshape(s.shape[0], -1), start, end, mask_1, curr_mask), dim=-1)
        if self.preproc:
            # Implement preproc described in Appendix A
        ...
        hidden0, cell0 = self.recurs(inp, (hidden[0], cell[0]))
        hidden1, cell1 = self.recurs2(hidden0, (hidden[1], cell[1]))
        return self.output(hidden1).exp(), (hidden0, hidden1), (cell0, cell1)
...
def test_optimizer(env, dim, Ptest, optim_it, Ntest, objective, optimizee_class, preproc=False, hidden_sz=20, load_net_path=None):
    N,K = dim
    test_data = load_test_data(N, K, Ntest)
...
def do_test_sumrate_vs_snr():
    ...
    res = test_optimizer((N,K), Ptest=Ptest, optim_it=3, test_data=test_data, objective=SumRateObjective, optimizee_class=OptimizeeMISO, hidden_sz=40, load_net_path=path)
