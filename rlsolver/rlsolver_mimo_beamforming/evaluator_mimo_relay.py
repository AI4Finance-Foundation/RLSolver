...
def evaluator_relay(policy_net_mimo_relay, K=4, N=4, M=4, P=10, noise_power=1, evaluate_H_path="H_K4N4M4.pkl", evaluate_G_path="G_K4N4M4.pkl", device=th.device("cpu")):
    ...
    with open(evaluate_H_path, 'rb') as f:
        evaluate_H = th.as_tensor(pkl.load(f), dtype=th.cfloat).to(device) 
    with open(evaluate_G_path, 'rb') as f:
        evaluate_G = th.as_tensor(pkl.load(f), dtype=th.cfloat).to(device)
