...
class Policy_Net_MIMO_Relay(nn.Module):
    ...
    def forward(self, state):
        ...
        mat_HF = torch.bmm(mat_H, mat_W.transpose(1,2).conj())
        vec_HF = torch.cat((mat_HF.real.reshape(-1, self.K * self.N), mat_HF.imag.reshape(-1, self.K * self.N)), 1)
        net_input = torch.cat((vec_H, vec_W, vec_HF), 1).reshape(-1, 6, self.K * self.N)
        net_input = net_input.reshape(-1, 6, self.K, self.N)
        vec_F_new = (self.sigmoid(self.net(net_input)) - 0.5) * 2
        vec_F_new = vec_F_new / torch.norm(vec_F_new, dim=1, keepdim=True)
