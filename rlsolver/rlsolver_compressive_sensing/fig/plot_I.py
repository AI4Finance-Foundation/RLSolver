...
with open(f"identity_{n_grad}_M={M}.pkl",'rb') as f:
    signal = pkl.load(f)
...
fig.savefig(f"origin_signal_{n_grad}_I_M={M}.png", bbox_inches='tight')
...
for row in range(4):
    ...
    ax[row].set_ylim(-1,1)
    ax[row].legend(loc="upper left")
fig.savefig(f"recovery_signal_{n_grad}_I_M={M}.png", bbox_inches='tight')
