...
class AgentPPO(AgentBase):
    ...
    def explore_one_env(self, env, target_step: int) -> list:
        ...
        min_H = 0
        for i in range(int(target_step)):
        #for i in tqdm(range(int(target_step))):
        #while i < target_step:
            
            states = torch.as_tensor(state, dtype=torch.float32).unsqueeze(0)
            actions, noises, actions_mid = [item.cpu() for item in get_action(states.to(self.device))]  # different
            next_state, reward, done, _ = env.step(convert(actions)[0].numpy())

            traj_list.append((states, reward, done, actions_mid, noises))  # different
    ...
    def update_net(self, buffer: ReplayBufferList):
        ...
        #print(buffer_size, self.batch_size)
