...
def train_optimizer():
    ...
    opt_iter = OptimizerIter(hid_dim=hid_dim).to(device)
    opt_base = optim.Adam(opt_iter.parameters(), lr=lr)
    ...
    for i in range(train_times + 1):
        ...
        opt_train(obj_func=obj_func, obj_args=(h_scale,), opt_task=opt_task, opt_iter=opt_iter,
            ...
            for i_p, p in enumerate(p_eval):
                ...
                for h in h_evals:
                    ...
                    best_result, min_loss = opt_eval(
                        obj_func=obj_func, obj_args=(h_scale,), opt_iter=opt_iter, opt_task=opt_task,
