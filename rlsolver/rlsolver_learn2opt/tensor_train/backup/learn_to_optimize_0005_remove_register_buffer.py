...
def run_opt(dim, opt_net, objective, meta_optimizer_class,
    ...
    for iteration in range(1, optim_it + 1):
        ...
        result_params = {name: p for name, p in meta_optimizer.named_parameters()}
        for name, p in meta_optimizer.named_parameters():
            ...
            temp = p + updates.view_as(p)
            result_params[name] = temp / temp.norm()
...
class MetaOptimizerMISO(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.N, self.K = dim
        self.theta = nn.Parameter(th.zeros(2 * self.N * self.K, device=DEVICE))

    def forward(self, target):
        w = self.theta.reshape((2, self.N, self.K))
        return target.objective_function(w[0] + 1j * w[1])


# class MetaOptimizerMISO0000000(nn.Module):
#     def __init__(self, dim):
#         super().__init__()
#         self.N, self.K = dim
